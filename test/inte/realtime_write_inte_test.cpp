...
class FailCommitReadersOnceRealtimeStore final : public DelegatingRealtimeStore {
 public:
    FailCommitReadersOnceRealtimeStore(const std::shared_ptr<RealtimeStore>& delegate,
                                       const std::shared_ptr<std::atomic<bool>>& fail_once)
        : DelegatingRealtimeStore(delegate), fail_once_(fail_once) {}

    Result<std::vector<std::unique_ptr<BatchReader>>> CreateCommitReaders(
        const std::shared_ptr<RealtimeSegmentHandle>& segment) override {
        if (fail_once_->exchange(false, std::memory_order_acq_rel)) {
            return Status::IOError("injected failure after sealing PK realtime data");
        }
        return delegate_->CreateCommitReaders(segment);
    }

 private:
    std::shared_ptr<std::atomic<bool>> fail_once_;
};
...
TEST_F(RealtimeWriteInteTest, TestPkPostSealFailureReplaysExternalWal) {
    CreatePkTable();
    const std::vector<Row> external_wal = {
        {1, "one", "p0"}, {1, "one-new", "p0"}, {2, "deleted", "p0"}, {3, "three", "p0"}};
    const std::vector<RecordBatch::RowKind> mutation_kinds = {
        RecordBatch::RowKind::INSERT, RecordBatch::RowKind::UPDATE_AFTER,
        RecordBatch::RowKind::DELETE, RecordBatch::RowKind::INSERT};
    std::shared_ptr<std::atomic<bool>> fail_once = std::make_shared<std::atomic<bool>>(true);
    std::shared_ptr<RealtimeStoreFactory> factory =
        MakeDecoratingFactory<FailCommitReadersOnceRealtimeStore>(fail_once);
    ...
    ASSERT_OK_AND_ASSIGN(
        std::unique_ptr<RecordBatch> failed_batch,
        MakeBatch(external_wal, /*partitioned=*/false, /*bucket=*/0, mutation_kinds));
    ASSERT_OK(failed_writer->Write(std::move(failed_batch)));
    ASSERT_NOK_WITH_MSG(failed_writer->PrepareCommitWithProgress(/*commit_identifier=*/0),
                        "injected failure after sealing PK realtime data");

    // The failed writer may already have sealed its process-local segment. Discard both objects,
    // reopen from the latest committed snapshot, and replay the caller-owned WAL.
    failed_writer.reset();
    ...
    ASSERT_OK_AND_ASSIGN(
        std::unique_ptr<RecordBatch> replay_batch,
        MakeBatch(external_wal, /*partitioned=*/false, /*bucket=*/0, mutation_kinds));
    ASSERT_OK(replay_writer->Write(std::move(replay_batch)));
    ASSERT_OK_AND_ASSIGN(std::vector<RealtimeCommitProgress> replay_progress,
                         replay_writer->PrepareCommitWithProgress(/*commit_identifier=*/0));
    ASSERT_EQ(1, replay_progress.size());
    ASSERT_EQ(OffsetRange(0, 4), replay_progress[0].offset_range);
    ASSERT_OK_AND_ASSIGN(int64_t snapshot_id, Commit(replay_progress, /*commit_identifier=*/0));
    ASSERT_OK(replay_writer->RefreshCommittedSnapshot(snapshot_id));
    const std::vector<Row> expected = {{1, "one-new", "p0"}, {3, "three", "p0"}};
    ASSERT_OK_AND_ASSIGN(std::vector<Row> memory_rows, ReadRows(replay_context));
