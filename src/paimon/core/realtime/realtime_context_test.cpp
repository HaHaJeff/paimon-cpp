...
class TestingMemIndexer : public MemIndexer {
 public:
    ...
    Status Reclaim(int64_t committed_offset) override {
        reclaimed_offsets.push_back(committed_offset);
        return Status::OK();
    }
    ...
    std::vector<int64_t> reclaimed_offsets;
};
...
TEST(RealtimeContextTest, TestCommittedProgressIsMonotonicAndSelective) {
    ...
    ASSERT_TRUE(factory->indexers[0]->reclaimed_offsets.empty());
    ASSERT_TRUE(factory->indexers[1]->reclaimed_offsets.empty());
    ...
    ASSERT_OK(context->AdvanceCommittedProgress(
        5, {RealtimePartitionBucketOffset{partition, /*bucket=*/0, /*offset=*/7},
            RealtimePartitionBucketOffset{{{"dt", "unknown"}},
                                          /*bucket=*/0,
                                          /*offset=*/9}}));
    ASSERT_EQ(std::vector<int64_t>({7}), factory->indexers[0]->reclaimed_offsets);
    ASSERT_TRUE(factory->indexers[1]->reclaimed_offsets.empty());
    ...
    ASSERT_EQ(std::vector<int64_t>({7}), factory->indexers[0]->reclaimed_offsets);
    ASSERT_EQ(std::vector<int64_t>({8}), factory->indexers[1]->reclaimed_offsets);
}
