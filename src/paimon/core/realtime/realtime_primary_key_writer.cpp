...
Result<std::shared_ptr<RealtimePrimaryKeyWriter>> RealtimePrimaryKeyWriter::Create(
    ...
    PAIMON_ASSIGN_OR_RAISE(RealtimePrimaryKeyMemIndexerState indexer_state,
                           realtime_context->GetOrCreatePrimaryKeyMemIndexer(
                               partition, bucket, std::move(write_schema), trimmed_primary_keys,
                               options, indexer_context));
