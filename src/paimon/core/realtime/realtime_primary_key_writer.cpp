...
Result<std::shared_ptr<MemIndexer>> CreatePrimaryKeyMemIndexer(
    ...
    auto io_manager = std::make_shared<IOManager>(temp_directory, file_system);
    PAIMON_ASSIGN_OR_RAISE(std::shared_ptr<PrimaryKeyMemIndexer> indexer,
                           PrimaryKeyMemIndexer::Create(
                               imported_schema, trimmed_primary_keys, key_comparator, merge_wrapper,
                               core_options, io_manager, enable_multi_thread_spill, memory_pool));
...
Result<std::shared_ptr<RealtimePrimaryKeyWriter>> RealtimePrimaryKeyWriter::Create(
    ...
    RealtimeContext::MemIndexerCreator creator =
        [&write_schema, &trimmed_primary_keys, &options, &memory_pool, &file_system,
         &temp_directory,
         enable_multi_thread_spill]() mutable -> Result<std::shared_ptr<MemIndexer>> {
        return CreatePrimaryKeyMemIndexer(std::move(write_schema), trimmed_primary_keys, options,
                                          memory_pool, file_system, temp_directory,
                                          enable_multi_thread_spill);
    };
    PAIMON_ASSIGN_OR_RAISE(RealtimeMemIndexerState indexer_state,
                           realtime_context->GetOrCreateMemIndexer(partition, bucket, creator));
